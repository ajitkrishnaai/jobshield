...
export default function BeforePhotosPage() {
  ...
  useEffect(() => {
    ...
    setJob(j);
    getPhotosForJob(jobId, 'before').then((p) => {
      setPhotos(p);
      setCurrentPromptIdx(p.length);
    });
  }, [jobId, router]);

  const prompts = useMemo(() => (job ? PHOTO_PROMPTS[job.trade].before : []), [job]);
  ...
  const handleCapture = useCallback(
    async (blob: Blob) => {
      ...
      if (currentPromptIdx + 1 >= prompts.length) {
        // All before photos taken
        saveJob({ ...job, status: 'in-progress', startedAt: new Date().toISOString() });
        setTimeout(() => router.push(`/app/job/${job.id}/progress`), 800);
      } else {
        setTimeout(() => {
    ...
    <div className="flex-1 flex flex-col bg-navy-dark">
      ...
      <div className="px-4 pt-4 pb-2">
        <div className="flex items-center justify-between mb-2">
          <div>
            <p className="text-orange text-xs font-semibold uppercase tracking-wide">Before Photos</p>
          ...
          <div className="flex gap-2 overflow-x-auto">
            {photos.map((p, i) => (
              <div key={p.id} className="w-12 h-12 rounded-lg overflow-hidden flex-shrink-0 border-2 border-white/20">
                {p.blob && (
                  <PhotoThumb blob={p.blob} alt={`Before ${i + 1}`} />
                )}
              </div>
            ))}
          </div>
